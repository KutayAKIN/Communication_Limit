...
def run_sim(opt,device):
    ...
    X_train,X_test,X_val,y_train,y_test,y_val = load_MNIST_dataset(opt.dataset_loc,0.1)
    ...
    for sim_i in sim_bar:
        try:
            ...
            Unif_Model = MNIST_Unif_Sim(params,device)
            Ind_Model = MNIST_Ind_Sim(params,device)
            Feed_Model = MNIST_Feed_Sim(params,device)
            Coop_Model = MNIST_Coop_Sim(params,device)
            Lwb_Model = MNIST_Lwb_Sim(params,device)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset-loc", type=str,default="./MNIST/data")
