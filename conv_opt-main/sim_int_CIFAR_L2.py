...
from CIFAR10_model import CIFARClassifier, ResNet18
...
X_train,X_test,X_val,y_train,y_test,y_val = load_CIFAR_dataset("./CIFAR/data",0.1)
...
train_dataset = create_CIFAR_traindataset(X_train[filt],y_train[filt])

val_dataset = create_CIFAR_dataset(X_val,y_val)

test_dataset = create_CIFAR_dataset(X_test,y_test)
...
if train_base  and not train_base_sim:
    ...
    dataloader, data, label = create_CIFAR_dataloader(train_dataset , b_size, device)

    losses = train_CIFAR_model(base_model,losses,loss_fn,optimizer,num_epoch,data,label,dataloader,False)
    ...
    plt.plot(torch.tensor(losses)[100:])

    plt.savefig("plots/cifar_train_loss.jpg")
...
while sim_i<n_sim:
    if 1:
        ...
        if train_base_sim:
            ...
            dataset_0 = create_CIFAR_base_dataset(X_train,y_train,base_classes,n_size)

            losses = []
            
            dataloader, data, label = create_CIFAR_dataloader(dataset_0 , b_size, device)

            losses = train_CIFAR_model(base_model,losses,loss_fn,optimizer,base_num_epoch,data,label,dataloader,True)
        ...
        base_model_ind = ResNet18().to(device)
        base_model_feed = ResNet18().to(device)
        base_model_coop = ResNet18().to(device)
        base_model_unif = ResNet18().to(device)
        ...
        if train_rounds:
            ...
            P_cond_ind = cond_preb_calc_CIFAR(base_model_ind,val_dataset,device)
            P_cond_coop = cond_preb_calc_CIFAR(base_model_coop,val_dataset,device)
            P_cond_feed = cond_preb_calc_CIFAR(base_model_feed,val_dataset,device)
            P_cond_unif = cond_preb_calc_CIFAR(base_model_unif,val_dataset,device)
...
if train_rounds:
    ...
    coop_acc_df.to_csv("data/CIFAR_Coop_Acc.csv")
    ind_acc_df.to_csv("data/CIFAR_Ind_Acc.csv")
    feed_acc_df.to_csv("data/CIFAR_Feed_Acc.csv")
    unif_acc_df.to_csv("data/CIFAR_Unif_Acc.csv")
    ...
    plt.tight_layout()
    plt.savefig("plots/sim_cifar_acc.jpg")
    ...
    coop_df.to_csv("data/CIFAR_Coop_L2.csv")
    ind_df.to_csv("data/CIFAR_Ind_L2.csv")
    feed_df.to_csv("data/CIFAR_Feed_L2.csv")
    unif_df.to_csv("data/CIFAR_Unif_L2.csv")
    lwb_df.to_csv("data/CIFAR_Lwb_L2.csv")
    ...
    plt.grid(linestyle='--', linewidth=2)
    plt.xlabel("Round $i$",fontweight="bold" ,fontsize=24)
    plt.ylabel("L2",fontweight="bold" ,fontsize=24)
    plt.rcParams["font.size"]=18
    plt.rcParams["axes.linewidth"]=2
    ax.xaxis.set_tick_params(labelsize=14)
    ax.yaxis.set_tick_params(labelsize=14)
    plt.legend(fontsize=20)
    plt.tight_layout()
    plt.savefig("plots/sim_cifar_action.jpg")