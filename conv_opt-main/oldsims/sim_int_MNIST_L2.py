...
if train_base  and not train_base_sim:
    ...
    dataloader, data, label = create_MNIST_dataloader(train_dataset , b_size, device)
...
while sim_i<n_sim:
    if 1:
        ...
        if train_base_sim:
            ...
            dataset_0 = create_MNIST_base_dataset(X_train,y_train,base_classes,n_size)

            losses = []
            
            dataloader, data, label = create_MNIST_dataloader(dataset_0 , b_size, device)

            losses = train_MNIST_model(base_model,losses,loss_fn,optimizer,base_num_epoch,data,label,dataloader,True)
        ...
        if train_rounds:
            ...
            P_cond_ind = cond_preb_calc_MNIST(base_model_ind,val_dataset,device)
            P_cond_coop = cond_preb_calc_MNIST(base_model_coop,val_dataset,device)
            P_cond_feed = cond_preb_calc_MNIST(base_model_feed,val_dataset,device)
            P_cond_unif = cond_preb_calc_MNIST(base_model_unif,val_dataset,device)
...
if train_rounds:
    ...
    coop_acc_df.to_csv("data/MNIST_Coop_Acc.csv")
    ind_acc_df.to_csv("data/MNIST_Ind_Acc.csv")
    feed_acc_df.to_csv("data/MNIST_Feed_Acc.csv")
    unif_acc_df.to_csv("data/MNIST_Unif_Acc.csv")
    ...
    plt.tight_layout()
    plt.savefig("plots/sim_mnist_acc.jpg")
